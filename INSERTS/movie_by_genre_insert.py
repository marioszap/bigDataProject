...
#SELECT movieId, AVG(rating) FROM ratings_by_date WHERE time_of_rating > '2015-1-1 00:00' AND time_of_rating < '2015-1-15 23:59' GROUP BY movieId ALLOW FILTERING;

#SELECT title, genre, avgRating, tag FROM movie_info WHERE title = 'Jumanji' ; 

#SELECT title FROM movie_by_genre WHERE genre = 'adventure';

#SELECT title FROM movie_by_title WHERE title = 'title:"star"';

#SELECT title, tag, avgRating FROM tags WHERE tag CONTAINS 'comedy' ORDER BY avgRating DSC LIMIT 20;
